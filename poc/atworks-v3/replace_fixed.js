...
const startIdx = lines.findIndex(l => l.includes('{action.navigations.map((nav: string, ni: number) => ('));
if (startIdx === -1) {
  console.log('Could not find start index for action navigations');
  process.exit(1);
}

// Find the corresponding closing of map
let endIdx = -1;
...
if (endIdx === -1) {
  console.log('Could not find end index for action navigations');
  process.exit(1);
}
...
console.log('Successfully replaced action navigations block with dynamic index.');
