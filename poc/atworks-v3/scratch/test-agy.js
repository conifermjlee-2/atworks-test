...
const tempDir = path.join(os.tmpdir(), `agy-test`);
...
fs.writeFileSync(promptFile, 'Return a JSON with a single scenario.', 'utf8');
fs.writeFileSync(schemaFile, '{ "type": "object", "properties": { "scenarios": { "type": "array" } } }', 'utf8');
...
& agy --output-format json --json-schema '${schemaFile}' --print $prompt *>&1 | Out-File -FilePath '${resultFile}' -Encoding utf8
`;
