...
/**
 * 기획서 6.4: Axios & Fetch Resolver (기본 활성화)
 * axios.get(), axios.post(), fetch() 등 순수 HTTP 클라이언트 추출
 */
export class AxiosFetchResolver implements HookResolver {
  resolve(calleeName: string, args: Array<t.Node | null>): ApiCallInfo | null {
    ...
    } else if (calleeName.startsWith('axios.') || calleeName.startsWith('api.') || calleeName.startsWith('client.')) {
      isAxiosOrFetch = true;
