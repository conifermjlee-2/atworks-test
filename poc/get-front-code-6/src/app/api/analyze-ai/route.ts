...
export async function POST(req: NextRequest) {
  try {
    ...
    const routeScenarios = body.routeScenarios;

    if (!routeScenarios || routeScenarios.length === 0) {
      return NextResponse.json({ error: 'No routeScenario data provided' }, { status: 400 });
    }

    // AI에게 전달할 데이터 압축 (토큰 절약을 위해 불필요한 필드 제거)
    const promptData = routeScenarios.map((r: any) => ({
      route: r.route,
      scenarios: r.scenarios.map((sc: any) => ({
        type: sc.triggerType,
    ...
    const prompt = `
다음은 쇼핑몰/웹 애플리케이션의 프론트엔드 화면(Route)별 API 호출 흐름(JSON)입니다.
...
다음 JSON 포맷에 맞추어 응답을 작성해 주세요. 어떠한 마크다운 백틱(\`\`\`json)이나 추가 설명 없이 순수한 JSON 문자열만 반환해야 합니다.
{
  "scenarios": [
    {
      "title": "이모지가 포함된 화면 기반 시나리오 제목 (예: 🛒 상품 탐색 화면)",
      "summary": "이 화면에서 일어나는 API 동작의 전체 목적을 설명하는 1~2줄의 간결한 요약",
      "tags": ["장바구니", "주문", "상품조회" 등 핵심 키워드 배열],
      "steps": [
        {
          "route": "입력받은 route 값",
          "apiFlow": "해당 화면에서 발생하는 핵심 API 흐름 (예: GET /api/cart ➞ POST /api/orders). 없으면 'API 호출 없음'으로 작성",
          "description": "이 API 흐름이 비즈니스적으로 어떤 동작인지 핵심만 1줄로 요약"
        }
      ]
    }
  ]
