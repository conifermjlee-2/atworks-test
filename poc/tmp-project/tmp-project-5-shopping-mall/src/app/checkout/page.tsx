 ...
 * [시나리오 2 - 2단계 API] GET /api/cart 호출 (주문할 상품 리스트 로드)
 * [시나리오 2 - 2단계 후행 액션] 약관동의 클릭 시 <Modal /> 공통 팝업 모달 오픈
 * [시나리오 2 - 3단계 API] POST /api/checkout 호출 (모의 결제 처리)
 * [시나리오 2 - 3단계 후행 액션] 승인 완료 후 /order-complete 주문 완료 B 화면으로 라우팅 이동
...
export default function CheckoutPage() {
  ...
  const [checkoutItems, setCheckoutItems] = useState<CartItem[]>(cart);

  // [시나리오 2 - 2단계 후행 액션: 약관 동의 공통 Modal 팝업 상태]
  ...
  // [시나리오 2 - 2단계 API 호출: GET /api/cart]
  useEffect(() => {
    async function loadCartData() {
      try {
        const serverCart = await fetchCartItems();
        if (serverCart && serverCart.length > 0) {
          setCheckoutItems(serverCart);
  ...
  // [시나리오 2 - 3단계 API 호출 및 후행 액션: POST /api/checkout ➔ /order-complete B 화면 이동]
  const handleFinalPayment = async (e: React.FormEvent) => {
    ...
    try {
      // 1. [시나리오 2 - 3단계 API 호출: POST /api/checkout]
      const response = await requestCheckoutApi({
        items: checkoutItems,
        shippingInfo,
        totalAmount,
        buyType: 'CART',
      ...
      if (response.success) {
        clearCart();
        // ⚡ 2. [시나리오 2 - 3단계 후행 액션]: 주문 완료 영수증 B 화면으로 라우팅 이동
        router.push(
        ...
        >
          <div style={{ fontSize: '0.9rem', color: 'var(--text-sub)', lineHeight: 1.6 }}>
            ...
            <p style={{ marginBottom: '0.8rem' }}>
              ...
              본 결제 시스템은 모의(Mock) 테스트용으로 동작하며, 실제 실제 금액이 청구되지 않습니다.
            </p>
            <button
