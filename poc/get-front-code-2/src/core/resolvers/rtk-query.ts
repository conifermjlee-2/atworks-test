...
export class RtkQueryResolver implements HookResolver {
  ...
  async init(rootDir: string): Promise<void> {
    ...
    const apiFiles = await fg([pattern1, pattern2], { ignore: ['**/*.test.*', '**/*.spec.*', '**/*.d.ts'] });
    ...
    for (const filePath of apiFiles) {
      try {
        ...
        const endpoints = extractEndpointBlocks(code);
        const hookExports = extractHookExports(code);
        
        for (const block of endpoints) {
          const hooks = hookExports.get(block.name) || generateHookNames(block.name, block.type);
...
function extractUrl(block: string): string {
  ...
  if (templateMatch) {
    return templateMatch[2].replace(/\$\{([^}]+)\}/g, (_, p1) => {
...
function extractHookExports(code: string): Map<string, string[]> {
  ...
  for (const hookName of hooks) {
    ...
    if (endpointName) {
      if (!map.has(endpointName)) map.set(endpointName, []);
      map.get(endpointName)!.push(cleanHook);
    }
  }
...
function generateHookNames(endpointName: string, type: 'query' | 'mutation'): string[] {
