...
import { useGetNoticesQuery, useUpdateUserMutation } from '../services/dashboard.api';

const api = axios.create({ baseURL: '/api/v1' });

export function Dashboard({ userId }: { userId: string }) {
  const [data, setData] = useState();
  const notices = useGetNoticesQuery();
  ...
  const save = () => {
    updateUser({ userId, name: 'Lee' });
  };

  return <button onClick={save}>Dashboard {notices.data?.length}</button>;
}
